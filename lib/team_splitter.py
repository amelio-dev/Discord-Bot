...
def set_team_num(message):
    pattern = r"\-num\s(\d+)"
    result = re.search(pattern, message.content)
    if result:
        return int(result.group(1))
    else:
        return DEFAULT_TEAM_NUM

def set_team_size(message):
    pattern = r"\-size\s(\d+)"
    result = re.search(pattern, message.content)
    if result:
        return int(result.group(1))
...
# 手動でユーザの追加、除外設定を行う場合
def modify_user_list(target_list, message):
    appending_users = []
    removing_users = []
    pattern = r"\-user\s\[(.+)\]"
    result = re.search(pattern, message.content)
...
def add_user(target_list, message):
    appending_users = []
    pattern = r"\-user\s([^\-]*)"
    result = re.search(pattern, message.content)
    if result:
...
def split_list(users, team_num):
    if len(users) < team_num:
        return split_list(users,len(users))
    else:
...
def is_team_command(message):
    return message.startswith('/team')

def create_teams(client, message):
    util = Util(client)
    ...
    member_names = modify_user_list(member_names,message)
    randomized_names = random.sample(member_names,len(member_names))

    team_num  = set_team_num(message)
    team_size = set_team_size(message)
    
    if "-lol" in message.content:
        if len(randomized_names) < 10:
            return "error: 人数が10人未満です。"
        teams = make_lol_team(randomized_names)
        mes = lol_result(teams)
    else:
        teams = split_list(randomized_names, team_num)
        mes = normal_result(teams)
    
    return create_team_headder(team_num, team_size) + mes

def create_team_headder(team_num, team_size):
